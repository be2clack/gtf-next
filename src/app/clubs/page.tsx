...
export const metadata: Metadata = {
  title: 'Клубы',
  description: 'Список клубов Таеквон-До ИТФ',
}
...
export default async function ClubsPage({ searchParams }: ClubsPageProps) {
  ...
  const regionId = params.regionId ? parseInt(params.regionId) : undefined
  ...
  const [clubs, total, regions] = await Promise.all([
    ...
    prisma.region.findMany({
      where: federation ? { countryId: federation.countryId } : {},
    ...
    <div className="container py-8">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        ...
        {regions.slice(0, 10).map((region) => (
          <Link key={region.id} href={`/clubs?regionId=${region.id}`}>
            <Badge
