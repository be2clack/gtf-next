...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: federation ? `Рейтинг - ${federation.name}` : 'Рейтинг спортсменов',
  }
}
...
export default async function FederationRatingsPage({ params, searchParams }: PageProps) {
  ...
  const discipline = searchParamsData.discipline || 'SPARRING'
  const gender = searchParamsData.gender
  const ageGroup = searchParamsData.ageGroup
    ...
    <div className="container py-8">
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {Object.entries(disciplineLabels).map(([key, label]) => (
          <Link key={key} href={`${baseUrl}?discipline=${key}${gender ? `&gender=${gender}` : ''}${ageGroup ? `&ageGroup=${ageGroup}` : ''}`}>
            <Badge
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        ...
        {Object.entries(genderLabels).map(([key, label]) => (
          <Link key={key} href={`${baseUrl}?discipline=${discipline}&gender=${key}${ageGroup ? `&ageGroup=${ageGroup}` : ''}`}>
            <Badge
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        ...
        {Object.entries(ageGroupLabels).map(([key, label]) => (
          <Link key={key} href={`${baseUrl}?discipline=${discipline}${gender ? `&gender=${gender}` : ''}&ageGroup=${key}`}>
            <Badge
